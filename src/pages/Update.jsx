...
const Update  = () => {
 ...
 const onSubmitHandler = async (event) => {
  ...
  try {
    ...
    const {data} = await axios.post(backendUrl + '/api/update-product', formData  )
    console.log(data)
    if(data.success){
      console.log(data)
      setDocImg(false)
  ...
  return (
    <form onSubmit={onSubmitHandler} className='m-5 w-full'>
        ...
        <p className='mb-3 text-lg font-medium'>Update Product</p>
        ...
        <div className='bg-white px-8  py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
          ...
          <div className='flex flex-col lg:flex-row items-start gap-10 text-gray-600'>
            <div className='w-full lg:flex-1 flex flex-col gap-4'>
              ...
              <div className='flex-1 flex flex-col gap-1'>
                <p>Id </p>
                <input onChange={(e)=> setId(e.target.value)} value={id} className='border rounded px-3  py-2' type='text' placeholder='Endter Id' required />
              </div>
              ...
              <button type='submit' className='bg-primary px-10 py-3 mt-4   rounded-full'>Update product</button>
